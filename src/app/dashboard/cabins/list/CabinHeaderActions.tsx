...
import { CabinFilterQuery, CabinQueryprops } from "../model/cabin.model";

const CabinHeaderActions: FC<CabinQueryprops> = ({ handleFilter }) => {
  const [query, setQuery] = useState<CabinFilterQuery | null>(null);
  async function handleClick(query: CabinFilterQuery | null) {
    setQuery(query);
    if (handleFilter) {
          ...
          <Menu.Items className="absolute z-50 right-0 mt-2 w-56 origin-top-right divide-y divide-gray-100 rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none">
            <div className="px-1 py-1 ">
              <Menu.Item>
                {({ active }) => (
                  <button
                    onClick={() => handleSortFilter("createdAt")}
              ...
              <Menu.Item>
                {({ active }) => (
                  <button
                    onClick={() => handleSortFilter("regularPrice")}
                    className="group text-black  hover:glass gap-1 flex w-full items-center rounded-md px-2 py-2 text-sm"
                  >
                    {active ? (
                      <BsSortUp color="green" />
